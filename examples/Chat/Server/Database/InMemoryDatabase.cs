...
public class InMemoryDatabase : IDatabaseAccess
{
    ...
    public AuthenticationResult Authenticate(string username, string password, out AccountUID accountUid)
    {
        if (_accountUidTable.TryGetValue(username, out AccountUID uid))
        {
            ...
            else
            {
                ...
                ScaleNetManager.Logger.LogWarning($"Account with username '{username}' has an {nameof(AccountUID)} but no {nameof(AccountData)}.");
            }
        }
