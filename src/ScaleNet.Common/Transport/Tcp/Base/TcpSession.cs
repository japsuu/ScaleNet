    ...
    internal class TcpSession : IAsyncSession
    {
        ...
        public SessionStatistics GetSessionStatistics()
        {
            ...
            return new SessionStatistics(
                messageBuffer.CurrentIndexedMemory,
                messageBuffer.CurrentIndexedMemory / (float)MaxIndexedMemory,
                totalBytesReceived,
                totalBytesSend,
                deltaReceived,
                deltaSent,
        ...
        {
            TransportLogger.Log(TransportLogger.LogLevel.Error, context + Enum.GetName(typeof(SocketError), e.SocketError));
            EndSession();
        }
...
#region Fields & Props
        ...
        private int SendBufferReleased;
        private int ReceiveBufferReleased;
...
#region Send
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void SendOrEnqueue(byte[] buffer, int offset, int count)
        {
            enqueueLock.Take();
            if (IsSessionClosing())
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void SendOrEnqueue(byte[] bytes)
        {
            enqueueLock.Take();
            if (IsSessionClosing())
                ReleaseSendResourcesIdempotent();
        ...
        private void SendComplete(object ignored, SocketAsyncEventArgs e)
        {
            try
            {
                ...
                if (e.BytesTransferred < e.Count)
                {
                    ...
                    if (!sessionSocket.SendAsync(e))
                    {
                        ...
                        TransportLogger.Log(TransportLogger.LogLevel.Info, "Resending");
                    }
            ...
            catch (Exception ex)
            {
                if (!IsSessionClosing())
                {
                    TransportLogger.Log(TransportLogger.LogLevel.Error, "Error on sent callback tcp session" + ex.Message);
                    EndSession();
                }
            }
        }
...
#region Dispose
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (!SendSemaphore.IsTaken())
                ReleaseSendResourcesIdempotent();
            SendSemaphore.Release();
            TransportLogger.Log(TransportLogger.LogLevel.Debug, string.Format("Session with Guid: {0} is disposed", SessionId));
        }
