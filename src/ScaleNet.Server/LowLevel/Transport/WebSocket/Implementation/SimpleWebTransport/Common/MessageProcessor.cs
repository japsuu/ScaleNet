...
public static class MessageProcessor
{
    ...
    public static bool NeedToReadShortLength(byte[] buffer)
    {
        ...
        return lenByte >= Constants.USHORT_PAYLOAD_LENGTH;
    }
    ...
    public static void ToggleMask(byte[] src, int srcOffset, byte[] dst, int dstOffset, int messageLength, byte[] maskBuffer, int maskOffset)
    {
        ...
        {
            byte maskByte = maskBuffer[maskOffset + i % Constants.MASK_SIZE];
    ...
    {
        if (lenByte == Constants.USHORT_PAYLOAD_LENGTH)
        {
        ...
        else if (lenByte == Constants.ULONG_PAYLOAD_LENGTH)
